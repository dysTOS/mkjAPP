export function dynamicWaveletAlgorithm(buffer: Float32Array, sampleRate: number): number {
  // https://github.com/peterkhayes/pitchfinder/blob/master/src/detectors/dynamic_wavelet.ts
  ...
  while (true) {
    ...
    for (let i = 2; i < curSamNb; i++) {
      const si = buffer[i] - theDC;
      const si1 = buffer[i - 1] - theDC;
    ...
    let newFloat32AudioBuffer = buffer.subarray(0);
    if (curSamNb === distances.length) {
      newFloat32AudioBuffer = new Float32Array(curSamNb / 2);
    }
    for (let i = 0; i < curSamNb / 2; i++) {
      newFloat32AudioBuffer[i] = (buffer[2 * i] + buffer[2 * i + 1]) / 2;
    }
    buffer = newFloat32AudioBuffer;
    curSamNb /= 2;
  }
