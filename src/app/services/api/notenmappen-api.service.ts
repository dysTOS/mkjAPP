...
export class NotenmappenApiService extends AbstractCrudApiService<Notenmappe> {
  ...
  public getNotenOfMappe(mappeId: string): Observable<Noten[]> {
    const url = environment.apiUrl + this.controllerApiUrlKey + '/noten/' + mappeId;
  ...
  public attachNotenToMappe(notenId: string, mappeId: string, verzeichnisNr: string = null): Observable<any> {
  ...
  public detachNotenFromMappe(notenId: string, mappeId: string): Observable<any> {
    const url = environment.apiUrl + this.controllerApiUrlKey + '/detach';
    return this.httpClient.post<any>(url, { noten_id: notenId, mappe_id: mappeId }, StandardHttpOptions);
  }
}
