...
export class TerminTeilnahmeSelectorComponent {
  ...
  public set termin(value: Termin) {
    ...
    if (dayjs(value?.vonDatum).isBefore(dayjs(new Date()).subtract(1, 'day'))) {
      this.disabled = true;
    }
  }
  ...
  public getStatus(): void {
    this.loading = true;
    this.teilnahmeService.getTeilnahmeStatus(this.termin.id).subscribe((data) => {
  ...
  public updateStatus(): void {
    this.loading = true;
    if (this.status) {
      this.teilnahmeService.updateTeilnahme(this.termin.id, this.status).subscribe({
        next: (data) => {
